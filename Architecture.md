...
| 14 | **Frontend: copy 100% UI app chính từ `aucobot/apps/web`**, bỏ auth + landing; sửa domain UI sau theo đề bài (§8.0) | Tiết kiệm thời gian scaffold; team FE có shell Telegram-style sẵn (sidebar, chat, composer) rồi mới gắn Dashboard agent |
...
> - Scaffold frontend từ trắng / chỉ Tailwind skeleton — loại; copy UI app chính Aucobot nhanh hơn và đồng bộ UX đã có.
>
...
**Component:** bước 1 **giữ nguyên stack styling Aucobot** — **Tailwind CSS v4** (`@import "tailwindcss"` trong `globals.css`) + components tự viết trong `components/`. Sau khi copy xong mới bổ sung panel Dashboard (DAG / approval / so sánh) bằng **cùng Tailwind**, không đổi hệ styling giữa chừng.

> **Chiến lược UI:** không scaffold từ trắng. Copy 100% app chính Aucobot (đã dùng Tailwind) rồi sửa theo đề bài — xem §8.0.
...
### 8.0 Chiến lược xây dựng frontend — copy UI từ Aucobot ✅

**Mục tiêu:** không mất thời gian dựng shell chat từ đầu. Lấy **toàn bộ UI ứng dụng chính** của Aucobot, bỏ phần không cần cho đề thi, rồi chỉnh dần theo Digital Expert Agents.

#### Copy gì / bỏ gì

| Nguồn `aucobot/apps/web` | Hành động | Lý do |
|---|---|---|
| `app/app/` — ClientAppShell, sidebar Room/Session, chat panel, composer, hash routing | **Copy 100%** | Đây là app chính — UX Telegram-style đã có |
| `components/` (ui, layout, chat, …) | **Copy 100%** | Design system + chat UI |
| `hooks/`, `stores/`, `lib/` (http, api, stream stubs), `schemas/`, `utils/`, `types/` | **Copy 100%** (rồi cắt auth client sau) | Giữ pipe FE mỏng: lib → hooks → stores → UI |
| `app/globals.css`, `layout.tsx`, `next.config.ts`, `proxy.ts` (đơn giản hóa) | **Copy rồi rút gọn** | Bỏ rewrite marketing/auth subdomain |
| `app/site/` — landing page | **Không copy / xóa** | Không cần marketing cho đề thi |
| `app/site/(auth)/` — login, register | **Không copy / xóa** | Đã chốt bỏ auth (§0 #11) |
| Mọi call auth (`lib/api/auth.ts`, auth-flow store, OTP UI, Google login) | **Gỡ sau khi copy** | Mở app vào thẳng shell chat / dashboard |
| Phụ thuộc `@aucobot/shared` | **Inline / copy schema cần thiết vào `frontend/`** | Repo thi không dùng packages workspace |

#### Quy trình làm việc (FE team)

```text
1. Copy cây apps/web → auco-team/frontend/ (**giữ Tailwind CSS v4**, cấu trúc folder)
2. Xóa app/site/** (landing + auth)
3. Đổi entry: mở / → thẳng ClientAppShell / TelegramAppShell (không redirect login)
4. Gỡ auth guard / credentials cookie bắt buộc — gọi API không cần session
5. Giữ nguyên UI chat (sidebar + thread + composer) làm khung demo
6. SAU ĐÓ mới sửa theo đề bài:
     - Đổi copy/branding → Digital Expert Agents / SHB demo
     - Thêm route hoặc panel Dashboard: DAG, timeline, approval, so sánh (§6) — **vẫn Tailwind**
     - Đổi conversation/Room mental model → TaskRun / yêu cầu liên phòng ban (khi sẵn sàng)
     - Nối API.md (mock → backend thật)
```

**Nguyên tắc:** bước 1–5 = **copy nguyên, chạy được shell**. Bước 6 = **customize theo đề bài** — không sửa nghiệp vụ agent trên FE; não vẫn ở `backend/`.

#### Lưu ý kỹ thuật khi copy

- Aucobot app chính đang dùng **Tailwind CSS v4** (`tailwindcss` + `@tailwindcss/postcss` trong `package.json`, `app/globals.css` có `@import "tailwindcss"`). Landing còn vài file CSS riêng — **không copy** vì đã bỏ `app/site/`.
- **Không “đổi sang Tailwind”** (vì đã là Tailwind) và **không rewrite sang CSS Modules** — copy nguyên hệ styling.
- `proxy.ts` / subdomain marketing: đơn giản hóa thành 1 host (dev `localhost`, prod 1 domain FE trên Vercel).
- Env: chỉ cần `NEXT_PUBLIC_API_URL` trỏ backend; bỏ biến auth Google/OTP.
- Một số file import `@aucobot/shared` — thay bằng copy type/Zod local trong `frontend/lib/schemas/` hoặc stub tạm để build pass.
...
|---|---|---|---|
| **Team frontend** | `frontend/` | Copy UI Aucobot (§8.0), gỡ auth/landing, mock API, sau đó Dashboard agent | Import code NestJS / Prisma / LLM / MCP |
...
├── frontend/                           # Next.js — copy từ aucobot/apps/web (§8.0), TEAM FRONTEND sở hữu
│   ├── package.json                    # độc lập
│   ├── src/ hoặc app/                  # giữ cấu trúc Aucobot sau khi copy
│   │   ├── app/
│   │   │   └── app/                    # ClientAppShell — KHÔNG có site/ auth/landing
│   │   ├── components/                 # ui, layout, chat (copy 100%)
│   │   ├── hooks/
│   │   ├── stores/                     # bỏ auth-flow
│   │   └── lib/
│   │       ├── api/                    # REST → NEXT_PUBLIC_API_URL (+ mock giai đoạn 1)
│   │       ├── stream/                 # WS client
│   │       ├── mock/
│   │       └── schemas/                # thay @aucobot/shared
│   └── .env.example                    # NEXT_PUBLIC_API_URL= (không auth secrets)
│
...
```text
Sprint 0 — Tách team + contract + copy UI
  Tạo folder frontend/ + backend/ (mỗi bên package.json riêng, KHÔNG workspace)
  FE: copy 100% UI app chính từ aucobot/apps/web → frontend/ (§8.0)
      xóa landing + auth; mở thẳng ClientAppShell; gỡ auth client
  Viết API.md (REST + WS tối thiểu §8.3) — FE mock theo đây
  BE: NestJS skeleton + Prisma + docker-compose Postgres

Sprint 1 — BE: khung dữ liệu + Planner · FE: shell Aucobot + mock
  BE: TaskRun/TaskStep + Planner generateObject + 1 Specialist Credit mock tool
  FE: giữ chat shell; bắt đầu gắn mock TaskRun vào UI (chưa cần đẹp Dashboard)
  Ghim kịch bản demo (§2.4) trên BE

Sprint 2 — BE: MCP + LlamaIndex RAG · FE: panel Dashboard trên shell đã copy
  BE: mcp-los + mcp-compliance; backend/src/rag (LlamaIndex); seed knowledge
  FE: thêm DAG / timeline / approval vào khung Aucobot; nối NEXT_PUBLIC_API_URL khi BE sẵn
...
### 13.10 “Frontend lấy từ đâu? Styling thế nào?”

> Nhóm **copy 100% UI app chính** từ `aucobot/apps/web`. App chính Aucobot đang dùng **Tailwind CSS v4** — copy nguyên, không đổi hệ styling. Bỏ landing và auth vì không thuộc đề bài. Sau khi shell chạy được mới chỉnh branding và thêm Dashboard agent (DAG, approval, so sánh) vẫn trên Tailwind. Cách này nhanh hơn scaffold từ trắng và giữ UX đã ổn định.
