...
> - Mesh agent↔agent tự do (CrewAI-style) — loại vì vòng lặp / khó audit; thay bằng fractal star có trần worker.
>
...
## 2. Lỗ hổng kỹ thuật trọng tâm: Routing ≠ Planning

Cơ chế điều phối gốc của Aucobot (`@Trợ Lý`) là **routing 1 tin → 1 agent** (lọc cứng theo `skillGroups`, chọn mềm theo `description`). Đề bài đòi **planning**: 1 yêu cầu phức tạp → tách thành nhiều tiểu-nhiệm-vụ, một số song song, một số phụ thuộc kết quả nhau, rồi tổng hợp.

**Ví dụ cần chạy được trong demo:**
...
Planner nhận goal → sinh TaskPlan (structured output, KHÔNG tự do văn bản):
  ...
  Step 1 [Credit Agent]      — song song ──┐
  Step 2 [Legal/Compliance]  — song song ──┼──► Step 4 [Product Agent]
                                            │      (chờ Step 1+2 xong, cần input)
  Step 3 [Legal/Compliance]  — song song ──┘
                                            ▼
...
### 2.1 Data model — `TaskRun` / `TaskStep` (mới, họ hàng ý tưởng `WorkflowRun` của Aucobot)
...
model TaskStep {
  ...
  // bỏ needsSubPlanning từ Planner — Specialist tự quyết Mode B
  input            Json
  output           Json?
...
4. Step có side-effect thật (submit hồ sơ, flag AML) → status = waiting_approval,
   chờ người duyệt (Approval queue, tái dùng nguyên triết lý Aucobot §5) mới chuyển done
5. Khi mọi step done → Planner step cuối "synthesize" → tổng hợp câu trả lời cuối + trích dẫn
```

**Guardrail bắt buộc** (tái dùng nguyên tắc Aucobot): giới hạn số step tối đa (chống plan vô hạn), timeout mỗi step, turn budget toàn `TaskRun` — tránh token nổ giống lý do Aucobot bỏ mesh agent-gọi-agent.

### 2.3 Mô hình multi-agent đã chốt — Specialist tự tạo worker rồi tổng hợp ✅

> **Verdict:** Đúng hướng bạn muốn. Agent chuyên gia **không chỉ nhận việc rồi gọi 1 tool** — khi cần đa nguồn / song song, nó **tự tạo worker tạm**, chạy song song, **tự gom dữ liệu**, rồi trả kết quả đã tổng hợp lên Planner. Đây là mô hình **"fractal star"** (hình sao lồng 1 cấp), không phải mesh.
...
#### Chế độ B — Specialist tự spawn (chi tiết implement)
...
Credit Agent nhận: "Đánh giá khả năng vay 2 tỷ của KH Nguyễn Văn A"
  ...
  → Planner KHÔNG cần biết W1/W2/W3 tồn tại (chỉ thấy output đã gom)
  → Dashboard VẪN hiện W1/W2/W3 lồng trong node Credit (trace / storytelling)
```

**Thuật ngữ chốt (để thuyết trình không bị hỏi xoáy):**

| Tên | Là gì | Sống bao lâu |
|---|---|---|
| **Planner** | Agent hệ thống điều phối liên domain | Cả `TaskRun` |
| **Specialist** | Agent chuyên gia cố định (Credit/Legal/Product/Ops) | Cả `TaskRun` / seed sẵn |
| **Worker** | "Agent tạm" do Specialist tạo cho 1 tiểu-nhiệm | **Chỉ trong 1 TaskStep** — xong thì huỷ, không vào danh bạ |

→ Khi giám khảo hỏi *"có tự tạo agent không?"*: **Có — Specialist tự spawn worker (ephemeral agents) cho tác vụ song song, rồi tự tổng hợp.** Không tạo vô hạn agent vĩnh viễn.

#### Giới hạn cứng (bắt buộc — nếu bỏ là demo dễ loạn)
...
| Ai quyết định Mode B | **Specialist tự quyết** (đọc mô tả TaskStep + skill của mình) | Đúng tinh thần "tự tạo"; không bắt Planner micromanage |
| Side-effect | Worker **chỉ đọc / phân tích**; mọi `mutates: true` vẫn do Specialist đề xuất → Approval | Không để worker tạm tự ghi vào hệ thống ngân hàng |
| Demo chính | **Credit Agent luôn vào Mode B** (ghim bằng few-shot/prompt) | Thuyết trình trôi chảy, DAG nhìn thấy spawn song song |

Trace: mỗi worker = 1 entry trong `TaskStep.toolCalls` dạng `{ type: "worker", workerId, goal, tool, input, output }` — **không thêm bảng Prisma**. Dashboard expand node Specialist là thấy ngay.

#### Vì sao không để Specialist gọi Specialist khác (mesh)?

Mesh Credit→Legal trực tiếp = vòng lặp + khó audit + Planner mất vai trò "người lập kế hoạch" theo đề bài. Fractal giữ: **Planner vẫn là planner liên phòng ban**; **Specialist chỉ spawn bên trong domain của mình** (đúng nghiệp vụ ngân hàng: chuyên viên tín dụng tự thu thập nhiều nguồn, không tự gọi luôn sang pháp chế thay quy trình).

---

Nguy cơ lớn nhất khi để Planner tự do sinh TaskPlan **trong lúc demo live**: LLM có thể ra plan khác mỗi lần chạy, có lúc lệch, gây mất kiểm soát trước giám khảo. Giải pháp — **không phải hard-code giả**, mà là **ghim kịch bản qua few-shot + kiểm tra trước**:

```text
1. Chọn CỐ ĐỊNH 1-2 "goal chính" cho demo (ví dụ đúng câu ở §2 đầu file)
2. Chạy thử Planner nhiều lần với goal đó → validate TaskPlan sinh ra ổn định
   (đúng số step, đúng agentRole, đúng dependsOn, đúng step nào gắn needsSubPlanning)
3. Đưa CHÍNH TaskPlan đã validate đó vào few-shot example trong system prompt của Planner
   → không ép cứng plan (Planner vẫn chạy generateObject thật), nhưng ghim rất chặt kết quả
     cho đúng câu hỏi demo — plan lệch xác suất thấp
4. Case ngoài kịch bản (giám khảo hỏi biến thể) vẫn chạy qua đúng engine chung — không giả,
   chỉ KHÔNG được đảm bảo mượt như case đã luyện
```

Mục tiêu: **thuyết trình trôi chảy cho case chính**, đồng thời **hệ thống vẫn tổng quát thật** (không phải demo giả lập cứng) khi bị hỏi xoáy.
...
### 13.4 “Tại sao không dùng CrewAI hoặc AutoGen?”
